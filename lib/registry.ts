...
import { items as RegistryIndex } from '@/registry.json';
import { registryItemFileSchema, registryItemSchema } from 'shadcn/registry';
import { Project, ScriptKind, SourceFile, SyntaxKind } from 'ts-morph';
import { z } from 'zod';

import { Index } from '@/registry/registry-component-index';

type ComponentsIndex = Record<
  string,
  {
    name: string;
    component: React.LazyExoticComponent<
      () => React.JSX.Element | Promise<React.JSX.Element | null>
    >;
  }
>;

const memoizedIndex = Object.fromEntries(
  RegistryIndex.map((registryItem) => [
    registryItem.name,
    {
      ...registryItem,
      files: registryItem.files.map((file) => ({
        ...file,
        type: file.type as z.infer<typeof registryItemSchema>['type'],
      })),
    },
  ])
);

const memoizedRegistryComponentIndex: ComponentsIndex = Object.fromEntries(
  Index.map((componentRegistryItem) => [
    componentRegistryItem.name,
    { ...componentRegistryItem },
  ])
);

export function getRegistryComponent(name: string) {
  return memoizedRegistryComponentIndex[name]?.component;
}
